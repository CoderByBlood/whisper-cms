...
use crate::db::sqlite::Bind;
use crate::db::sqlite::SqliteDbError; // writer-actor Bind type
use adapt::db;
use adapt::db::Database;
use adapt::db::SqlValue;
use sqlx::Column;
use sqlx::Row;
use sqlx::Sqlite;
use sqlx::Type;
...
pub fn inject_database(db_url: &str) -> Database {
    Database::Injected(
        db_url.to_string(),
        exec_batch_write,
        exec_fetch_all,
...
fn exec_batch_write(db_url: &str, statements: Vec<(String, Vec<SqlValue>)>) -> db::Result<usize> {
    run_async(async {
...
) -> db::Result<Vec<HashMap<String, SqlValue>>> {
    run_async(async {
        // Map adapter-level SqlValue → infra-level Bind
...
fn checkpoint_wal(db_url: &str) -> db::Result<()> {
